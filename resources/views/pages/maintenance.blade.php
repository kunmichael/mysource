...
@section('content')
	...
	<div class="content-wrapper">
		...
		<div class="row">
			<div class="col-sm-12 col-md-12 col-lg-12">
				...
				<div class="box-header with-border">
				  <h3 class="box-title"><b>Data Maintenance</b></h3>
						...
						<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 table-responsive">
							<div class="nav-tabs-custom">
								<ul class="nav nav-tabs">
									<li class="active"><a href="#tab_1" data-toggle="tab">ALL <small class="label pull-right bg-blue" style="margin-left:5px;">{{$count_all_maintenances}}</small></a></li>
									<li><a href="#tab_2" data-toggle="tab">Approve <small class="label pull-right bg-green" style="margin-left:5px;">{{$count_approve_maintenances}}</small></a></li>
									<li><a href="#tab_3" data-toggle="tab">Need Approval <small class="label pull-right bg-red" style="margin-left:5px;">{{$count_need_approval_maintenances}}</small></a></li>
								</ul>
								<div class="tab-content">
									<div class="tab-pane active" id="tab_1">
										<table id="all_maintenances" class="table table-bordered table-hover">
											<thead>
												<tr>
													<th>Nama</th>
													<th>Subjek</th>
											...
											<tbody>
												...
												?>
												@foreach($all_maintenances as $row)
													...
													<td>
														<div class="btn-group">
															...
															<ul class="dropdown-menu pull-right" role="menu">
																...
																<li>
																	<a href="detail_maintenance_report/{{ $row->Report_maintenance_ID }}"><span class="glyphicon glyphicon-list-alt" style="color:blue;"></span>Detail Laporan</a>
																</li>
																<li>
																	<a href="#detail_member" data-toggle='modal' onclick="detail_member({{ $row->Report_maintenance_ID }})"><i class="fa fa-group" style="color:blue;"></i>Detail Member</a>
																</li>
																@if($row->Approval == 0)
																	<li>
																	<a href="approve_maintenance_report/{{ $row->Report_maintenance_ID }}"><i class="fa fa-check"></i>Approve</a>
																	</li>
																@endif
																@if((Session::get('user_login_data')->Jabatan_ID == 1 || Session::get('user_login_data')->Jabatan_ID == 4 || Session::get('user_login_data')->Jabatan_ID == 5 || Session::get('user_login_data')->Jabatan_ID == $karyawan_object::find($row->Karyawan_ID)->Jabatan_ID) && $row->Approval == 0 )
																	<li>
																	<a href="update_maintenance_report/{{ $row->Report_maintenance_ID }}"><i class="fa fa-edit" style="color:green;"></i>Ubah</a>
																	</li>
																@endif
																@if((Session::get('user_login_data')->Jabatan_ID == 1 || Session::get('user_login_data')->Jabatan_ID == 4 || Session::get('user_login_data')->Jabatan_ID == 5 || Session::get('user_login_data')->Jabatan_ID == $karyawan_object::find($row->Karyawan_ID)->Jabatan_ID) && $row->Approval == 0)
																	<li>
																	<a href="delete_maintenance_report/{{ $row->Report_maintenance_ID }}"><i class="fa fa-trash-o" style="color:red;"></i>Hapus </a>
																	</li>
																@endif
									...
									<div class="tab-pane" id="tab_2">
										<table id="approve_maintenances" class="table table-bordered table-hover">
											<thead>
												<tr>
													<th>Nama</th>
													<th>Subjek</th>
											...
											<tbody>
												<?php 
													...
													$project_object = new \App\Model\Project(); 
												?>
												@foreach($approve_maintenances as $row)
												<tr>
													<td>{{$karyawan_object::find($row->Karyawan_ID)->Nama_karyawan}}</td>
													<td>{{$row->Subject}}</td>
													<td>{{$project_object::find($row->Project_ID)->Nama_project}}</td>
									...
									<div class="tab-pane" id="tab_3">
										<table id="need_approval_maintenances" class="table table-bordered table-hover">
											<thead>
												<tr>
													<th>Nama</th>
													<th>Subjek</th>
											...
											<tbody>
												<?php 
													...
													$project_object = new \App\Model\Project(); 
												?>
												@foreach($need_approval_maintenances as $row)
												<tr>
													<td>{{$karyawan_object::find($row->Karyawan_ID)->Nama_karyawan}}</td>
													<td>{{$row->Subject}}</td>
													<td>{{$project_object::find($row->Project_ID)->Nama_project}}</td>
		...
		<script>
			...
			$("#all_maintenances, #approve_maintenances, #need_approval_maintenances").dataTable({
				"bPaginate": true,
				"bLengthChange": true,
				"bFilter": true,
				"bSort": true,
			...
			function detail_member(ID){
				...
				if (ID=="")
				{
					...
					xmlhttp.open("GET","get_detail_member_report_maintenance/"+ID,true);
					xmlhttp.send();
				}
			}
		</script>
